...
function EnvelopeCover({ clicked, onOpen }: { clicked: boolean; onOpen: () => void }) {
  const reduce = useReducedMotion()

  const container: Variants = {
    show: {
      transition: { staggerChildren: reduce ? 0 : 0.16, delayChildren: reduce ? 0 : 0.15 },
    },
  }
  ...
  return (
    ...
    >
      <img src={envelopeBg} alt="" aria-hidden="true" className="absolute inset-0 h-full w-full object-cover" />
      ...
      >
        ...
        <motion.div variants={item} className="my-4 sm:my-5 w-14 h-px" aria-hidden="true">
          <span className="block w-full h-full" style={{ background: 'linear-gradient(90deg, transparent, rgba(181,154,99,0.6), transparent)' }} />
        </motion.div>
        ...
        >
          Mariel Alyssa{' '}
          <span className="not-italic" style={{ fontFamily: "'Great Vibes', cursive", fontWeight: 400, color: GOLD_LINE }}>
            &amp;
          </span>{' '}
          Ivan Alexis
        </motion.h1>
        ...
        <motion.p
          ...
          style={{ fontFamily: "'Great Vibes', cursive", fontWeight: 400, color: GOLD_TEXT }}
        >
        ...
        >
          <motion.span
            ...
            style={{ background: 'radial-gradient(circle, rgba(181,154,99,0.5) 0%, rgba(247,241,231,0.35) 45%, transparent 70%)' }}
          ...
          <motion.p
            ...
            style={{ fontFamily: "'Cormorant Garamond', serif", fontWeight: 600, color: MAUVE }}
          >
        ...
        <motion.p
          variants={item}
          className="mt-7 sm:mt-8 text-[clamp(1rem,4.5vw,1.4rem)] tracking-[0.18em]"
          style={{ fontFamily: "'Cormorant Garamond', serif", fontWeight: 600, color: BURGUNDY }}
        >
          13 · 02 · 2027
        </motion.p>
      </motion.main>
